...
def train_one_epoch_classic(net, criterion, data_loader, optimizer, device, epoch, conf, log_writer=None):
    """
    ...
    for data_it, (img, label) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        image_patches = img.to(device, dtype=torch.float32)
...
def evaluate(net, criterion, data_loader, device, conf, header):
    ...
    Recall_metric = torchmetrics.Recall(task='multiclass',num_classes=conf.n_class, average='macro').to(device)
    Recall_metric(y_pred, y_true)
    recall = Recall_metric.compute().item()
    Specificity_metric = torchmetrics.Specificity(task='multiclass',num_classes=conf.n_class, average='macro').to(device)
...
# Disable gradient calculation during evaluation
@torch.no_grad()
def evaluate_classic(net, criterion, data_loader, device, conf, header):
    # Set the network to evaluation mode
    net.eval()
    ...
    for (img, label) in metric_logger.log_every(data_loader, 100, header):
        image_patches = img.to(device, dtype=torch.float32)
        labels = label.to(device)
        # coords = data['coords']
