...
def split_dataset_inflam(file_path, conf):
    h5_data = h5py.File(file_path, 'r')
    split_file_path = './splits/%s/split_%s.json'%(conf.dataset, conf.seed)
    if os.path.exists(split_file_path):
        with open(split_file_path, 'r') as json_file:
...
def build_HDF5_feat_dataset(file_path, conf):
    ...
    elif conf.dataset in ['inflam', 'four']:
        train_split, train_names, val_split, val_names, test_split, test_names = split_dataset_inflam(file_path, conf)
        train_split, train_names = generate_fewshot_dataset(train_split, train_names, num_shots=conf.n_shot)
        return HDF5_feat_dataset2(train_split, train_names), HDF5_feat_dataset2(val_split, val_names), HDF5_feat_dataset2(test_split, test_names)
...
class PNG_dataset(Dataset):
    def __init__(self, png_root, splits):
    ...
    def __getitem__(self, index):
        name, label = self.splits[index]
        fn = os.path.join(self.png_root, name + '.png')
...
def build_PNG_dataset(png_root, conf):
    if conf.arch == "Vit":
        dataset_custom = PNG_dataset_Vit
    else:
        dataset_custom = PNG_dataset

    if conf.dataset == 'camelyon':
        csv_path = "splits/camelyon/splits_0_label.csv"
        df = pd.read_csv(csv_path, dtype=str, na_filter=False)
        train_split = [(name, int(label)) for name, label in zip(df['train'], df['train_label']) if name and label]
        val_split = [(name, int(label)) for name, label in zip(df['val'], df['val_label']) if name and label]
        test_split = [(name, int(label)) for name, label in zip(df['test'], df['test_label']) if name and label]

        return dataset_custom(png_root, train_split), dataset_custom(png_root, val_split), dataset_custom(png_root, test_split)
        
        # train_split, train_names, val_split, val_names, test_split, test_names = split_dataset_camelyon_png(png_root, conf)
    elif conf.dataset == 'inflam':
        csv_path = "splits/inflam/split_label.csv"
        df = pd.read_csv(csv_path, dtype=str, na_filter=False)
        train_split = [(name, int(label)) for name, label in zip(df['train'], df['train_label']) if name and label]
        val_split = [(name, int(label)) for name, label in zip(df['val'], df['val_label']) if name and label]
        test_split = [(name, int(label)) for name, label in zip(df['test'], df['test_label']) if name and label]
