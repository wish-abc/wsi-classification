...
parser.add_argument('--config', dest='config', default='config/inflam_medical_ssl_config.yml',
...
if __name__ == '__main__':
    ...
    cat = args.data_slide_dir.split('/')[-1]
    output_path = os.path.join(cfg.data_dir, f'{cat}_patch_feats_pretrain_{cfg.pretrain}.h5')
    h5file = h5py.File(output_path, "w")
    # cat_to_label = {"SK":0, "Bowen":1, "BCC":2, "SCC":3}
    cat_to_label = {"海绵":0, "扁平苔藓":1, "银屑病":2}
    for bag_candidate_idx in range(total):
        # slide_id = bags_dataset[bag_candidate_idx].split(args.slide_ext)[0]
        slide_id = bags_dataset[bag_candidate_idx].split('/')[-1].split('.')[0]
        ...
        slide_grp.attrs['label'] = cat_to_label[cat]
